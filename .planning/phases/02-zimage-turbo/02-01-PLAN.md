...
must_haves:
  truths:
    - "Z-Image Turbo loader node appears in ComfyUI node browser under MFlux/ZImage category"
    ...
    - "Legacy MfluxZImageNode preserved for backward compatibility"
  artifacts:
  ...
  key_links:
    ...
    - from: "Mflux_Comfy/Mflux_ZImage.py"
      to: "temp file cleanup"
      via: "finally block in MfluxZImageImg2Img"
      pattern: "finally:\\s*.*os\\.remove"
---
...
- CATEGORY: "MFlux/ZImage" (follows existing "MFlux/Air" and "MFlux/Pro" naming convention)
- FUNCTION: "load_model"
- Implementation:
...
- Implementation:
  ...
  - Use try/finally pattern for temp file cleanup:
    ```python
    temp_path = None
    try:
...
**Imports at top (with environment variable guards):**
...
- Conditional mflux imports with guards:
  ```python
  ...
  if not _skip_mflux_import:
      try:
          from mflux import ZImageTurbo, ModelConfig
      except ImportError:
          ZImageTurbo = None
          ModelConfig = None
  else:
...
"
```
Note: Both MFLUX_COMFY_DISABLE_MLX_IMPORT and MFLUX_COMFY_DISABLE_MFLUX_IMPORT env vars are implemented in the codebase (see Mflux_Pro.py, Mflux_Core.py, tests/conftest.py) and will be supported by this module.
  </verify>
  <done>
...
- All three classes have CATEGORY = "MFlux/ZImage" (consistent with existing "MFlux/Air", "MFlux/Pro" pattern)
- Uses Phase 1 utilities for tensor conversion and memory management
- Temp file cleanup handled via try/finally block in MfluxZImageImg2Img
- Environment variable guards implemented for testing support
  </done>
</task>
...
IMPORTANT: Keep existing MfluxZImageNode registration for backward compatibility. The new nodes are ADDITIONS, not replacements. This preserves existing workflows that use the legacy combined node.
...
# Verify categories (consistent with MFlux/Air, MFlux/Pro pattern)
assert MfluxZImageLoader.CATEGORY == 'MFlux/ZImage'
assert MfluxZImageSampler.CATEGORY == 'MFlux/ZImage'
assert MfluxZImageImg2Img.CATEGORY == 'MFlux/ZImage'
...
- INFRA-05: Nodes appear under "MFlux/ZImage" category (consistent with existing naming)
...
- [ ] All three nodes have CATEGORY = "MFlux/ZImage"
- [ ] Loader returns ZIMAGE_MODEL type
- [ ] Samplers return IMAGE type
- [ ] Uses Phase 1 utilities (tensor_utils, memory_utils)
- [ ] Temp file cleanup in MfluxZImageImg2Img via try/finally
- [ ] Environment variable guards for MLX and mflux imports
